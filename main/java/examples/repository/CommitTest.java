...
public class CommitTest implements CreatedFiles {

    private final File file;
    private SvnCheckout checkout;

    public CommitTest(File f) {
        file = f;
    }
    ...
    private static void commitExample() throws SVNException, IOException {
        ...
        commitEditor.addFile("someFile", null, SVNRepository.INVALID_REVISION);
    ...
    private void createFile(ISVNEditor commitEditor, String fileName) {
        try {
            SVNDeltaGenerator deltaGenerator = new SVNDeltaGenerator();
            commitEditor.addFile(fileName, null, SVNRepository.INVALID_REVISION);
            commitEditor.applyTextDelta(fileName, null);
            byte[] iotaFileText = fileName.getBytes();
            String checksum = deltaGenerator.sendDelta(fileName, new ByteArrayInputStream(iotaFileText), commitEditor,
                    true);
    ...
    private static long calculateSize(Path pathCopiedDirectoryForRepo) {
        try {
            ...
            return Files.list(pathCopiedDirectoryForRepo).mapToLong(CommitTest::calculateSize).sum();
        } catch (IOException e) {
            return 0L;
        }
    }
